...
app.get("/editValues", function (req, res) {
  var data = req.query;
  var id = data.id;
  var blurVal = data.blur;
  var brightnessVal = data.brightness;
  var contrastVal = data.contrast;
  var grayscaleVal = data.grayscale;
  var hueRotateVal = data.hueRotate;
  var invertVal = data.invert;
  var opacityVal = data.opacity;
  var saturateVal = data.saturate;
  var sepiaVal = data.sepia;

  db.collection("img").findOne({id:id}, function(err, result){
      result.blur = blurVal;
      result.brightness = brightnessVal;
      result.contrast = contrastVal;
      result.grayscale = grayscaleVal;
      result.hueRotate = hueRotateVal;
      result.invert = invertVal;
      result.opacity = opacityVal;
      result.saturate = saturateVal;
      result.sepia = sepiaVal;
